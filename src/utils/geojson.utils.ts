...
// GeometryCollection is checked recursively since booleanValid does not support it.
// Null-safe; any thrown error is treated as invalid.
export const isValidGeometry = (geometry: Geometry | null | undefined): boolean => {
...
// Deterministic, unambiguous polygon defects that always also fail isValidGeometry.
// Used only to produce a more specific user-facing message; anything else falls back
// to the generic invalid-geometry message.
export type PolygonDefect = 'UNCLOSED_RING' | 'TOO_FEW_POINTS' | 'DUPLICATE_VERTICES';

// Compares two positions by X/Y only. This layer is 2-D: upload geometries are stripped of
// any Z coordinate (removeExtraCoordDimensionsIfNeeded) before defect detection runs.
const positionsEqual = (a: Position, b: Position): boolean => a?.[0] === b?.[0] && a?.[1] === b?.[1];

const getRingDefect = (ring: Position[]): PolygonDefect | null => {
  if (!Array.isArray(ring) || ring.length < 4) {
    return 'TOO_FEW_POINTS';
  }
  // Defects are reported in a fixed order (too-few-points, then unclosed, then duplicate);
  // a ring with more than one defect surfaces only the first match, which is enough to pick a
  // helpful message.
  if (!positionsEqual(ring[0], ring[ring.length - 1])) {
    return 'UNCLOSED_RING';
  }
  for (let i = 1; i < ring.length; i++) {
    if (positionsEqual(ring[i], ring[i - 1])) {
...
// Returns the first deterministic defect found in a Polygon/MultiPolygon's rings, or null
// (non-polygon types, valid rings, or an invalidity we cannot attribute with certainty).
export const getPolygonDefect = (geometry: Geometry | null | undefined): PolygonDefect | null => {
  if (!geometry) {
    return null;
  }
  let rings: Position[][] = [];
  ...
  for (const ring of rings) {
    const defect = getRingDefect(ring);
    if (defect) {
      return defect;
    }
  }
