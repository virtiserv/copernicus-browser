...
import { PAGE_SIZE } from '../api/OData/ODataHelpers';
import { formatSearchResults } from '../api/OData/searchResults.utils';
...
interface ODataSearchError extends Error {
...
not always correct.
*/
const hasMore = (results: { value?: unknown[] } | null | undefined): boolean => {
...
export const useODataSearch = () => {
  const [query, setQuery] = useState<ODataQueryBuilder | null>(null);
  const [oDataSearchResult, setODataSearchResult] = useState<ODataSearchResult | null>(null);
  const [searchInProgress, setSearchInProgress] = useState(false);
  const [searchError, setSearchError] = useState<ODataSearchError | null>(null);
  ...
  const search = useCallback(
    async (query: ODataQueryBuilder | null, results: ODataSearchResultItem[] = [], page = 0) => {
      if (!query) {
      ...
      try {
        ...
        if (!(data && data.value && data.value.length)) {
          ...
          if (results && results.length) {
            const result: ODataSearchResult = {
              allResults: results,
              page: page,
              hasMore: false,
          ...
          const error: ODataSearchError = new Error(errorMessage);
        ...
        */
        ...
        setSearchError(e as ODataSearchError);
      } finally {
        setSearchInProgress(false);
      }
    },
  ...
  */
  ...
  return [{ searchInProgress, searchError, oDataSearchResult }, setQuery, setAuthToken, hydrate] as const;
};
